...
export const toggleLikeService = async (
  ...
  try {
    // Capitalize for MongoDB enum
    const dbTargetType = targetType === "video" ? "Video" : "Comment"
    ...
    await Like.create(
      [{ userId, targetId, targetType: dbTargetType }],
      { session }
    )
  ...
  return Like.aggregate([
    {
      $match: {
        ...
        targetType: "Video"  // Capitalized to match enum
      }
    },
